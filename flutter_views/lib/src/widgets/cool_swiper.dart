...
class _CoolSwiperState extends State<CoolSwiper>
  ...
  Widget _buildBackgroundCardsStack() {
    return Stack(
      children: List.generate(
        _cards.isNotEmpty ? _cards.length - 1 : 0,
  ...
  Widget _buildStackChild(int i) {
    return Positioned(
      ...
      right: 0,
      child: IgnorePointer(
        ignoring: stackChildren.isNotEmpty ? i != stackChildren.length - 1 : true,
        child: CoolSwiperCardWrapper(
          animationController: backgroundCardsAnimationController,
          initialScale: _cards.isNotEmpty ? _cards[i].scale : 1,
          initialYOffset: _cards.isNotEmpty ? _cards[i].yOffset : 0,
          child: stackChildren.isNotEmpty ? stackChildren[i] : Container(),
        ),
      ),
    );
  }
